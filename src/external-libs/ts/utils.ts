...
function compareElementaryTypes(val1: any, val2: any, type: string): boolean {
   ...
   if (type.match(/^bytes\d*$/) || type.match(/^byte$/) || type.match(/^address$/)) {
      ...
      if(typeof val1 === "string" && typeof val2 === "string") {
         let lower1 = val1.toLowerCase();
         let lower2 = val2.toLowerCase();
...
export function isSupportedBasicSolidityType(type: string): boolean {
   // 
   return !!type.match(/^u?int\d+$/) || !!type.match(/^bool$/) || !!type.match(/^bytes\d*$/) || !!type.match(/^byte$/) || !!type.match(/^address$/) || !!type.match(/^string$/);
}
...
export function encodeAttestationType(attestationTypeName: string) {
   ...
   let bytes = ethers.toUtf8Bytes(attestationTypeName);
   if (bytes.length > 32) {
...
export function structsDeepEqual(struct1: any, struct2: any, abi: ABIFragment): boolean {
   ...
   for (const item of abi.components || []) {
      const key = item.name;
      let val1 = struct1[key];
      let val2 = struct2[key];
