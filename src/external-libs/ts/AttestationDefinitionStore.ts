...
export class AttestationDefinitionStore {
    ...
    * @returns 
    */
   attestationResponseHash(response: any, salt?: string): string | null | undefined {
      const attestationType = decodeAttestationType(response.attestationType);
      let definition = this.getDefinitionForDecodedAttestationType(attestationType);
      if (!definition) {
   ...
   encodeRequest(request: ARBase): string {
      let attestationType = decodeAttestationType(request.attestationType);
      let definition = this.getDefinitionForDecodedAttestationType(attestationType);
      if (!definition) {
   ...
   parseRequest<AR extends ARBase>(bytes: string, decodeAttestationTypeName = false): AR {
      let prefix = this.extractPrefixFromRequest(bytes);
      let attestationType = decodeAttestationType(prefix.attestationType);
      let definition = this.getDefinitionForDecodedAttestationType(attestationType);
      if (!definition) {
         throw new Error(`Unsupported attestation type id: '${attestationType}'`);
      }
